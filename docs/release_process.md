...
This document outlines the steps to release a new version of **azure-functions-logging** to PyPI and update the changelog using the existing Makefile and Hatch-based workflows.
...
1. Update the version in `src/azure_functions_logging/__init__.py`
...
```bash
pip install dist/azure_functions_logging-<version>-py3-none-any.whl
```
...
```bash
pip install --index-url https://test.pypi.org/simple/ azure-functions-logging
```
...
- [CHANGELOG.md](https://github.com/yeongseon/azure-functions-logging/blob/main/CHANGELOG.md)
